...
export default function HomeScreen() {
  ...
  useEffect(() => {
    if (user && profile) {
      ...
      const shouldShowTutorial =
        profile.last_tutorial_shown_month === null ||
        profile.last_tutorial_shown_month !== currentMonth;
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const imagePromises = todayExp.map(async (expense) => {
        ...
        return { expenseId: expense.id, imageUrl: images[0]?.urls.small || '' };
      });
  ...
  const handleSeeAllTodayExpenses = () => {
    ...
    router.push({ pathname: '/expenses', params: { dateFilter: today } });
  };
  ...
  const handleScrollRight = () => {
    if (expensesScrollViewRef.current) {
      const cardWidth = 280;
      expensesScrollViewRef.current.scrollTo({ x: cardWidth, animated: true });
    }
  };
  ...
  const styles = StyleSheet.create({
    ...
    expensesCarouselContainer: { position: 'relative' },
    expensesScrollView: { flex: 1 },
    expensesScrollContent: { paddingRight: 60 },
    scrollButton: {
      position: 'absolute',
      right: 16,
      top: '50%',
    ...
    expenseAmount: { fontSize: 18, fontWeight: '700', color: theme.error },
    todayBanner: {
      backgroundColor: theme.surface,
      borderRadius: 12,
      padding: 16,
    ...
    todayBannerText: { fontSize: 16, color: theme.text, textAlign: 'center' },
    todayBannerHighlight: { fontWeight: '700', color: theme.primary },
    ...
    quickActionCardPro: { borderWidth: 2, borderColor: theme.secondary },
    ...
    proBadgeText: { fontSize: 10, fontWeight: '700', color: '#ffffff' },
    ...
    addOptionsTitle: { fontSize: 20, fontWeight: '700', color: theme.text },
    closeOptionsButton: { padding: 4 },
    addOptionButton: {
      flexDirection: 'row',
      alignItems: 'center',
      backgroundColor: theme.surface,
    ...
    addIncomeIcon: { backgroundColor: `${theme.success}20` },
    addExpenseIcon: { backgroundColor: `${theme.error}20` },
    ...
    addOptionSubtext: { fontSize: 14, color: theme.textSecondary },
  });

  return (
    <ConversationalAIProvider onSuccess={loadData}>
                ...
                <>
                  ...
                  <View style={styles.todayBanner}>
                    <Text style={styles.todayBannerText}>
                      Today's Total:{' '}
                      <Text style={styles.todayBannerHighlight}>
                        ${todayTotal.toFixed(2)}
                      </Text>
                      {'\n'}
                      Current Remaining:{' '}
                      <Text style={styles.todayBannerHighlight}>
                        ${(remainingBudget - todayTotal).toFixed(2)}
                      </Text>
                    </Text>
                  </View>
                </>
